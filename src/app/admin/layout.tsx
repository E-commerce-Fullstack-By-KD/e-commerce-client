  ...
  return (
    <div className="flex min-h-screen bg-[#0f1117]">
        ...
        <div className="border-t border-white/10 p-3 space-y-1">
          ...
          {!collapsed && user && (
            <div className="flex items-center gap-2 rounded-lg px-3 py-2">
              <div className="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-gradient-to-br from-orange-400 to-amber-500 text-xs font-bold text-white">
              ...
              <button onClick={logout} title="Logout" className="text-slate-500 hover:text-red-400">
                <svg className="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                  <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2}
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        ...
        <header className="flex h-16 items-center justify-between border-b border-white/10 bg-[#16181f] px-6">
          <div>
            <h1 className="text-sm font-semibold text-white capitalize">
              {sidebarLinks.find((l) => isActive(pathname, l.href, l.exact))?.label ?? "Admin"}
            </h1>
        ...
        <main className="flex-1 overflow-auto bg-[#0f1117] p-6">{children}</main>
      </div>
    </div>
  );
}
