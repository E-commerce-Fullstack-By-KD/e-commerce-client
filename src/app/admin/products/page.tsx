...
export default function AdminProductsPage() {
  ...
  const f = (key: keyof CreateProductPayload) => (v: string) =>
    setForm((prev) => ({
      ...prev,
      [key]: key.includes("price") || key === "stock" ? Number(v) : v,
    }));
  ...
  const fetchProducts = useCallback(async () => {
    setFetching(true);
    try {
      const res = await get<{ result?: { products?: AdminProduct[] } }>(
      ...
      {showForm && (
        <div className="rounded-xl border border-orange-500/20 bg-[#16181f] p-6">
          ...
          <div className="mt-5 flex justify-end gap-2">
            <AdminBtn variant="ghost" onClick={() => setShowForm(false)}>
              Cancel
            </AdminBtn>
              ...
              <tbody className="divide-y divide-white/5">
                {products.map((p, i) => (
                  <tr key={p.id} className="hover:bg-white/5 transition-colors">
                    ...
                    <td className="px-5 py-3 font-semibold text-orange-400">
